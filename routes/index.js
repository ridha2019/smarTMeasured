...
var serviceAccount = require("../antennameasured-1871cf731a74.json");
...
router.get('/', function(req, res, next) {
   //console.log('-in routes-router.get * '+req.session.userId);
	if (req.session.userId) {
		...
		res.render('measured',{admin: isAdmin});
    }
...
router.post('/sessionLogin/', function(req, res, next) {
	...
	console.log('In router.post user 1*2 '+email);
...
/* --------------------------------------------------------------------- */
router.get('/measured/', function(req, res, next) {
	if (req.session.userId) {
        //isAdmin=req.session.userdroits;
		//console.log(isAdmin,' = ',req.session.userdroits);
		return res.render('measured',{admin: isAdmin});
    }
    res.render('index', {page:'Home', menuId:'home'});
})
router.post('/measured/', (req, res) => {
	if (req.session.userId) {
		res.redirect('/templates_measured/');
    }
    res.render('index', {page:'Home', menuId:'home'});	
})
router.get('/templates_measured/', (req, res) => {
	res.render('templates/measured');
})
/* --------------------------------------------------------------------- */
...
router.post('/getUserProducts/', (req, res) => {
	...
	var docRef1 = db1.ref("measured");
	docRef1.once("value", function(snapshot) {
		snapshot.forEach(function(doc) {
			//console.log(doc.val().Famil_prod);
			//console.log(d.indexOf(doc.val().Famil_prod));
			if (parseInt(d.indexOf(doc.val().Famil_prod))<0) {
				d.push(doc.val().Famil_prod);
			}
		});
    	res.setHeader('Content-Type', 'application/json');
...
/* --------------------------------------------------------------------- */
router.post('/getmesureData/', (req, res) => {
	var d = [];
	var devRef = req.body.Famil_prod;
	var docRef1 = db1.ref("measured");
	docRef1.once("value", function(snapshot) {
		snapshot.forEach(function(doc) {
			if(doc.val().Famil_prod == devRef) {
				d.push(doc.val());
			}
		});	
    	res.setHeader('Content-Type', 'application/json');
...
/* --------------------------------------------------------------------- */
router.post('/getDataMeasure/', (req, res) => {
	var _ref = [];
	var data_from=req.body.data_from;
	var first=req.body.datedeb;
	var second=req.body.datefin;
